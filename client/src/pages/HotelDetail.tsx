    ...
    const fetchHotelData = async () => {
      ...
      try {
        ...
        if (foundHotel) {
          ...
          console.log('Found hotel rooms_available:', foundHotel.rooms_available); // Debug logging
          console.log('Found hotel roomNormalizedDescription:', foundHotel.roomNormalizedDescription); // Debug logging
          
          if (foundHotel.price) {
            totalPrice = Math.round(foundHotel.price as number);
          } else if (foundHotel.lowest_price) {
            totalPrice = Math.round(foundHotel.lowest_price as number);
          }
          ...
          const pricePerNight = numNights > 0 ? Math.round(totalPrice / numNights) : totalPrice;
          
          console.log('Total price for stay:', totalPrice); // Debug logging
          console.log('Number of nights:', numNights); // Debug logging
          console.log('Price per night:', pricePerNight); // Debug logging
          
          // Use roomNormalizedDescription for room type, fallback to a better default
          roomType = foundHotel.roomNormalizedDescription || 'Standard Room';
          freeCancellation = foundHotel.free_cancellation || false;
          
          console.log('Extracted price for hotel:', pricePerNight); // Debug logging
          console.log('Room type:', roomType); // Debug logging
          console.log('Available rooms:', foundHotel.rooms_available); // Debug logging
          ...
          roomData = [{
            ...
            key: foundHotel.id,
            rooms_available: foundHotel.rooms_available || 1 // Add available rooms count
          }];
        }
        ...
        if (roomData.length === 0) {
          ...
          if (firstHotelWithPrice) {
            ...
            const fallbackTotalPrice = Math.round((firstHotelWithPrice.price || firstHotelWithPrice.lowest_price) as number);
            
            // Calculate price per night for fallback hotel too
            const checkinDate = new Date(checkin);
            const checkoutDate = new Date(checkout);
            const numNights = Math.round((checkoutDate.getTime() - checkinDate.getTime()) / (1000 * 60 * 60 * 24));
            const fallbackPricePerNight = numNights > 0 ? Math.round(fallbackTotalPrice / numNights) : fallbackTotalPrice;
            
            roomData.push({
              id: firstHotelWithPrice.id,
              room_type: firstHotelWithPrice.roomNormalizedDescription || 'Standard Room',
              price: fallbackPricePerNight,
              ...
              rooms_available: firstHotelWithPrice.rooms_available || 1
            });
          } else {
            ...
            roomData.push({
              ...
              amenities: ['WiFi', 'TV', 'Air Conditioning'],
              key: hotelId,
              rooms_available: 0 // No rooms available when no pricing data
            });
          }
        }
        ...
        const totalAvailableRooms = roomData.length > 0 ? (roomData[0].rooms_available || 1) : 0;
        ...
        console.log('Number of rooms available:', totalAvailableRooms); // Debug logging
