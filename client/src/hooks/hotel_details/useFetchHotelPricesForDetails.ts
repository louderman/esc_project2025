...
const priceDetailsCache = new Map<string, Price[]>();
const SAFE_PRICE_COUNT = 3;

function useFetchHotelPricesForDetails(
  destIds: string[],
    ...
    try {
      ...
      await Promise.all(
        destIds.slice(0, maxParallelFetchCount).map(async (destId) => {
          if (cache && priceDetailsCache.has(destId)) {
            allPrices.push(...priceDetailsCache.get(destId)!);
            return;
          }
          ...
          if (data.completed) {
            ...
            if (cache) {
              priceDetailsCache.set(destId, data.hotels);
            }
          } else {
            allCompleted = false;
          }
      ...
      if (cache) {
        for (const destId of destIds.slice(
          ...
          if (priceDetailsCache.has(destId)) {
            allPrices.push(...priceDetailsCache.get(destId)!);
          }
        }
      }
...
export { useFetchHotelPricesForDetails, priceDetailsCache };
