...
const hotelDetailsCache = new Map<string, Hotel[]>();
const SAFE_HOTEL_COUNT = 3; // to prevent getting blocked...

function useFetchHotelsForDetails(
  destIds: string[],
      ...
      try {
        ...
        await Promise.all(
          destIds.slice(0, maxParallelFetchCount).map(async (destId) => {
            if (options?.cache && hotelDetailsCache.has(destId)) {
              allHotels.push(...hotelDetailsCache.get(destId)!);
              return;
            }
            ...
            if (options?.cache) {
              hotelDetailsCache.set(destId, data);
            }
          })
        );
        ...
        if (cache) {
          for (const destId of destIds.slice(
            ...
            if (hotelDetailsCache.has(destId)) {
              allHotels.push(...hotelDetailsCache.get(destId)!);
            }
          }
        }
...
export { useFetchHotelsForDetails, hotelDetailsCache };
