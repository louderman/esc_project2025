...
describe('User Model', () => {
  ...
  test(
    'Input: Non-existing ("Shawn","Shawn@gmail.com","Shawn@123") → Output: Returns ID',
    async () => {
      const [result]: any = await insertUser('Shawn', 'Shawn@gmail.com', 'Shawn@123');
      ...
      expect(rows[0].password).toBe('Shawn@123');
    }
  );

  test(
    'Input: Existing ("Shawn","Shawn@gmail.com","Shawn@123") again → Output: Error due to unique email',
    async () => {
  ...
  test(
    'Input: One string empty ("", "Shawn@gmail.com", "Shawn@123") → Output: validation error message',
    () => {
      expect(() => insertUser('', 'Shawn@gmail.com', 'Shawn@123')).toThrow(
  ...
  test(
    'Input: Two strings empty ("", "", "Shawn@123") → Output: validation error message',
    () => {
      expect(() => insertUser('', '', 'Shawn@123')).toThrow(
        'All fields (name, email, password) must be non-empty and trimmed.'
      );
    }
  );
  ...
  test(
    'Input: All three empty ("", "", "") → Output: validation error message',
    () => {
  ...
  test(
    'Input: Existing "Alice@gmail.com" → Output: user object {id,name,email,password}',
    async () => {
      const [ins]: any = await insertUser('Alice', 'Alice@gmail.com', 'Alice@123');
